# Vendored helpers from Depth-Anything-3
# (Depth-Anything-3/src/depth_anything_3/model/utils/head_utils.py).
# Subset kept: Permute, custom_interpolate, create_uv_grid,
# position_grid_to_embed, make_sincos_pos_embed. Used only by the depth
# head (`models/dpt.py`); copying instead of importing because DA3 is not
# pip-installable from its current layout.
#
# Apache-2.0 license, original copyright (c) 2025 ByteDance Ltd.
...
def create_uv_grid(
    ...
    """Create a normalised UV grid of shape (width, height, 2)."""
...
def custom_interpolate(
    ...
    """Safe interpolation that chunks along batch to avoid INT_MAX overflow."""
    if size is None:
